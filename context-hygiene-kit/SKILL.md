...
2. **Two-channel boundary (LSC-7).** Only the **trusted channel** (user + assistant text) is harvested; `tool_result`/`tool_use` blocks are never ingested. Untrusted card content is rendered inside `<data>…</data>` in the digest — the curator *ranks* card content, never executes it. This is the prompt-injection defense.
