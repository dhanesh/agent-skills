...
2. Each refuter defaults to `refuted: true` when uncertain. The burden of proof is on the finding, not the refuter. A refuter that crashes or returns nothing is maximally uncertain, so it counts as `refuted: true`, and so does a missing vote when fewer than 3 (or fewer than the declared number of refuters) are recorded.
3. **Aggregation.** Downgrade to `UNCONFIRMED` when ≥2 of the 3 refuters return `refuted: true`. Because a 2-of-3 vote over a same-model ensemble is the weak baseline, calibrate by severity: require **unanimity (3-of-3 *non*-refute)** to keep a `critical`/`high`-severity `VIOLATION` or `DEVIATION`, and weight a refuter's vote by the strength of the evidence it brings (a refuter that cites a fetched source over-application beats one that only asserts doubt). This trades a little recall for precision exactly where a false positive is most costly. A finding with a missing or unknown severity takes the unanimity rule too: the rubric fails closed. Evidence weighting can only add a downgrade; it never rescues a finding the vote count downgrades — the linter counts votes.
4. The refutation outcome — number of refuters, individual verdicts, dominant rationale — is recorded in the `refutation` field of the finding object. The field is required on every surviving `VIOLATION`/`DEVIATION`. `assets/report_lint.py` rejects a survivor with no recorded votes, or whose recorded votes step 3 says downgrade it: ≥2 refutes on a `medium`/`low` finding, or any refute on a finding of any other severity (`critical`, `high`, missing or unknown). The linter checks that the recorded votes agree with the verdict. It cannot prove the votes were honest, and the evidence-weighting in step 3 stays a judgment the refuters make.
