...
2. Each refuter defaults to `refuted: true` when uncertain. The burden of proof is on the finding, not the refuter. A refuter that crashes or returns nothing is maximally uncertain, so it counts as `refuted: true`, and so does a missing vote when fewer than 3 are recorded.
