...
`settings.json` (existing hooks preserved), gitignore `.world-model/`, and **run the 21-test
...
Always confirm the gate passed: `python3 test_world_model.py` (21 tests — the two-axis
invariant, noisy-OR derivation, soft-invalidation, contradiction detect + propose, trust
boundary, idempotent ingest, referent mapping, measurable improvement). If any fail, the
...
- `assets/test_world_model.py` — the 21-test install gate.
