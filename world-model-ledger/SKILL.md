...
`settings.json` (existing hooks preserved), gitignore `.world-model/`, and **run the 43-test
...
0. **(Optional) Seed the model repo-wide.** On a fresh install, run
   ...
   `depends_on` edges, across Python, Ruby, JavaScript/TypeScript, Go, Rust, and the DevOps
   stack (Dockerfile, docker-compose/K8s, GitHub Actions, Terraform, Make, shell). Everything is
...
Always confirm the gate passed: `python3 test_world_model.py` (43 tests — the two-axis
...
- `assets/test_world_model.py` — the 43-test install gate.
