...
- **Status detection** — `agent-status-scan` inspects recent pane output and classifies each agent as `blocked`, `working`, `idle`, `done`, or `unknown` based on output patterns and explicit `AGENT_STATUS:` markers.
...
- **Jump-to-status** — `agent-jump blocked|working|idle|done` switches focus directly to the first pane in that state; `prefix g` / `prefix W` / `prefix I` / `prefix D` bind the four states to keys.
