...
- **mockstar CLI surface (verified, do not invent):** `mockstar init [dir]`, `mockstar serve [config-root]` (default; flags `--handlers`, `--port`, `--host`, `--deterministic`, `--no-watch`), `mockstar import <spec> <out-dir>` (OpenAPI 3.x only), `mockstar enhance <mocks-dir>` (flags `--dry-run`, `--spec=<file>`). Invoke via `bunx mockstar`.
