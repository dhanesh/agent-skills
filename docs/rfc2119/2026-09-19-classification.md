...
|---|---|---|---|---|---|
| c1 | 39 | Build the change-detector that unblocks agent work on an untested codebase. This is **not… | MUST (0.34, 0.79) | plain | the "never" describes the deliverable; the no-coverage-percentage rule itself is stated in Deliverable |
| c2 | 77 | The ranker and the python, go and rust guards are stdlib-only python3 — the go guard driv… | MUST (0.84, 0.88) | plain | both nevers describe guard behaviour (offline, never fetches); no never/always sentence here is a directive |
| c3 | 109 | **On go, make sure the module's dependencies are already downloaded.** The go guard runs… | MUST (0.41, 0.63) | plain | both nevers describe the go guard (never fetches; exit 5 never a verdict); the imperative is a workflow step |
| c4 | 114 | **On rust, check the lockfile, the toolchain and the cargo cache before you plan to write… | MUST (0.48, 0.81) | MUST |  |
| c5 | 131 | Offline, stdlib + git only, deterministic. Read `references/parameters.md` for the full a… | MUST (0.61, 0.75) | plain | "always"/"never" describe the ranker (rust reads heuristic, never runs cargo); the imperatives are workflow steps |
| c6 | 148 | Every unit lands in exactly one of the four testability tiers described in full in `refer… | SHOULD (0.39, 0.69) | plain | a summary of the tier outcomes; "reported, never applied" is carried as MUST NOT by Invariant 1 |
| c7 | 164 | The ranker's triage is conservative on purpose — it would rather under-tier a unit than o… | MUST (0.72, 0.70) | MUST |  |
| c8 | 193 | **The runtime guard, not the tier, is what enforces "never real I/O."** Static triage is… | MUST (0.94, 0.87) | plain | both nevers describe the guard (quoted invariant name; never written into the repo); Invariant 2 carries the MUST NOT |
| c9 | 203 | On **python** that is `assets/io_guard.py`, loaded as a **pytest plugin via `-p`** (never… | MUST (0.63, 0.77) | plain | describes how the guard loads (plugin, not a conftest.py or fixture); eval check 19 needs the negation wording, which stays |
| c10 | 246 | **On node, read the exit status, not the per-test results.** When a violation lands *afte… | MUST (0.74, 0.81) | plain | "Never the test that caused it" describes node --test attribution, not a directive |
| c11 | 271 | - **You MUST NOT keep a test reported `ok` from a run that exited nonzero.** | MUST (0.69, 0.78) | MUST |  |
| c12 | 295 | **Copy the whole block.** `<package>` is the package directory (`./internal/billing`), an… | MUST (0.60, 0.75) | plain | restates Invariant 1 (append, never overwrite), which already carries the MUST NOT; one keyword per rule |
| c13 | 336 | **Copy the whole block.** Run it from the crate root, adding `-p <package>` for a workspa… | MUST (0.52, 0.73) | plain | restates Invariant 1 (append, never overwrite), which already carries the MUST NOT; one keyword per rule |
| c14 | 381 | - A unit the ranker tiers 3 as *not reachable* or *binary-only* is reported, never tested. | MUST (0.51, 0.76) | plain | states the triage outcome for an unreachable unit; a test for one cannot be proven (Invariant 3 carries the MUST NOT) |
| c15 | 392 | - A compile error is exit 5, never RED; cargo exits 101 for both. | MUST (0.47, 0.79) | plain | defines the exit mapping (compile error is exit 5, never RED); descriptive |
| c16 | 394 | - a dependency not in the local cargo cache (run `cargo fetch`, or build the tests once;… | MUST (0.52, 0.80) | plain | "the proof never downloads anything" describes the guard |
| c17 | 409 | - The guard never passes `--nocapture`, and you MUST NOT add it: under it the default… | MUST (0.67, 0.84) | MUST |  |
| c18 | 418 | - **Tier 2 candidate:** blocks the uncontrollable groups always, plus every controllable… | MUST (0.64, 0.81) | plain | "blocks ... always" describes the Tier 2 guard; the "name all of it" imperative is a workflow step |
| c19 | 424 | The guard patches the **lowest** layer reachable, which for CPython is the `os` primitive… | MUST (0.87, 0.83) | plain | "never routes through" describes CPython internals |
| c20 | 478 | 1. **You MUST NOT modify source.** You MUST only create test files. When a test file… | MUST (0.90, 0.85) | MUST |  |
| c21 | 481 | 2. **You MUST NOT write a test that performs real I/O.** Enforced by the tier-aware… | MUST (0.94, 0.83) | MUST |  |
| c22 | 484 | 3. **You MUST NOT ship an unproven test.** A test that did not go RED MUST be discarded… | MUST (0.88, 0.66) | MUST |  |
| c23 | 486 | 4. **You MUST NOT leave the suite red.** End state is a green suite plus suspected bugs… | MUST (0.92, 0.77) | MUST |  |
| c24 | 488 | 5. **Hard gate before writing** — step 3's confirmation MUST happen before any test file… | MUST (0.86, 0.76) | MUST | (no departure) changed in 1.4.0 (autonomy-grant Task 7, now line 489): "…MUST happen before any test file is touched, unless `check-grant … --action local_reversible` exits 0; then you MAY write tests without asking, and MUST name the grant id and action class in the report. A grant never lifts Invariant 1." Still MUST: the grant is the one release, and it never reaches source edits |
| c25 | 499 | You MUST emit every captured value with `repr()`. You MUST NOT build a test's expected… | MUST (0.97, 0.82) | MUST |  |
| c26 | 535 | **Promoted units** get a line in the report naming the ranker's original tier, the tier u… | MUST (0.73, 0.44) | plain | harm 0.44 caps it at SHOULD, and a SHOULD here would contradict step 2's MUST NOT (c7) for the same rule; left as its restatement |
| c27 | 538 | **How to improve this.** `inbound_refs` is a static approximation — an identifier-occurre… | plain (0.36, 0.16) | plain |  |
| c28 | 170 | 3. **Confirm with the user before writing anything.** … This is a hard gate — you MUST NOT proceed past it unconfirmed, unless `check-grant --root <repo> --action local_reversible` exits 0 …; then you MAY proceed without asking, and MUST name the grant id and action class in the report. | not judged (new in 1.4.0, autonomy-grant Task 7) | MUST | the step-3 write gate (design spec §5): was plain "do not proceed past it unconfirmed"; keyworded to match Invariant 5 (c24) now that it carries the grant clause; counted once at MUST, the MAY being the option the grant opens; eval checks 53-54 grade both places. Final fix wave (M-c): the fallback reads "Any other exit (3 ASK/NONE, 2 INVALID, 1 usage error) means ask as usual"; plain, counts unchanged |
...
| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 95 | \| **PostToolUse** → `hooks/posttooluse-observe.sh` \| after **every** tool call \| the univ… | MUST (0.82, 0.79) | plain | the "never" describes what the PostToolUse hook reads (tool input, not output); Invariants 2 and 3 carry the rule |
| c2 | 101 | The model maintains itself: the universal `PostToolUse` hook captures entities and behavi… | MAY (0.41, 0.28) | plain | the only never/always sentence ("A model never guesses facts inside a hook") describes the hook design; Invariant 2 carries the MUST NOT |
| c3 | 109 | 0. **(Usually automatic) Seed the model repo-wide.** `SessionStart` auto-seeds a fresh re… | plain (0.31, 0.28) | plain |  |
...
| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 28 | Your job with this skill: turn a fuzzy "make it keep going until it's good" request into … | MUST (0.49, 0.85) | plain | the never/always list describes the properties of a sound loop; step 4 non-negotiables carry the MUSTs |
| c2 | 36 | Ask the user (or infer, then state your assumption): *what is this loop trying to achieve… | MUST (0.69, 0.74) | plain | "can never legitimately stop" is rationale for the done test; the imperative is a workflow step |
| c3 | 51 | \| LSC-2 \| Stop condition (primary) \| how the model signals "done" (a flag/token the harne… | MUST (0.79, 0.85) | MUST |  |
...
| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 23 | The defining rule: **no fabricated endpoints.** Every mock traces back to a fetche… | MUST (0.92, 0.83) | plain | restates Invariant 1 (no fabricated endpoints), which carries the MUST NOT; one keyword per rule |
| c2 | 48 | \`bunx @dhaneshpurohit/mockstar\` available (install: \`bun add -g @dhaneshpurohit/mock… | MUST (0.74, 0.80) | MUST |  |
...
| c5 | 68 | 3. **Schema-valid output.** Verification (Stage 5) boots the server. A mock proje… | MUST (0.84, 0.74) | MUST |  |
| c6 | 71 | 4. **No silent truncation.** When \`--max-endpoints\` caps the inventory, every drop… | MUST (0.87, 0.76) | MUST |  |
...
| c8 | 84 | \`--runtime auto\|local\|docker\` — selects how mockstar is invoked (default: \`auto\`)… | MUST (0.15, 0.74) | plain | describes what each \`--runtime\` value does; not a directive to the agent (confidence 0.15) |
...
| c17 | 332 | Write `MOCKSTAR-COVERAGE.md` to the output directory root, following the template in `references/coverage-report.md`, with these sections: | plain (0.25, 0.45) | plain | backfill 2026-09-25: was lowercase "The report must include:". Confidence 0.25 < 0.6 (SHOULD 0.41), decided ourselves: a workflow step (spec §2: steps stay plain imperatives); harm 0.45 < 0.5 |

## clean-code (34 candidates · 0 MUST · 13 SHOULD · 0 MAY · 21 plain · 7 departures)

| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 31 | A working guide to Robert C. Martin's principles. The aim is not to recite rules — … | plain (0.94, 0.16) | plain |  |
| c2 | 36 | But *clean* means **as simple as the problem allows**, never as elaborate as possi… | plain (0.57, 0.20) | plain |  |
...
| c4 | 75 | 1. Run the suite. Green? Refactor. Red? Fix or report the failure first — you SHOULD NOT… | MUST (0.55, 0.43) | SHOULD | harm 0.43 < 0.5 caps it below MUST; no gate enforces it; 'fix or report' is the stated alternative |
| c5 | 145 | The **principle and the shape** are the point, never the syntax. A snippet in one … | plain (0.80, 0.16) | plain |  |
| c6 | 156 | \| Rule \| One-line \| Violation signal \| | plain (0.68, 0.23) | plain | table header, not a sentence |
| c7 | 157 | \|---\|---\|---\| | plain (0.83, 0.16) | plain | table separator, not a sentence |
| c8 | 158 | \| **Meaningful names** \| Names SHOULD reveal intent \| `d`, `tmp`, `data`, `obj` \| | SHOULD (0.18, 0.22) | SHOULD |  |
| c9 | 159 | \| **Small functions** \| A function SHOULD do ONE thing, and do it well \| Function > ~20 lines \| | SHOULD (0.20, 0.34) | SHOULD |  |
| c10 | 160 | \| **No side effects** \| A function either DOES or ANSWERS — it SHOULD NOT do both \|… | SHOULD (0.24, 0.28) | SHOULD |  |
| c11 | 161 | \| **DRY** \| You SHOULD NOT repeat yourself \| Copy-paste with minor edits \| | SHOULD (0.35, 0.37) | SHOULD |  |
| c12 | 162 | \| **No magic numbers** \| Literals SHOULD be named \| `if (x > 86400)` \| | SHOULD (0.25, 0.44) | SHOULD |  |
| c13 | 163 | \| **Fail fast** \| Code SHOULD validate early and throw exceptions, not codes \|… | SHOULD (0.20, 0.29) | SHOULD |  |
| c14 | 164 | \| **Boy Scout Rule** \| You SHOULD leave code cleaner than you found it \| No cleanup before commit \| | SHOULD (0.32, 0.30) | SHOULD |  |
| c15 | 168 | 1. **Small** — functions SHOULD NOT exceed ~20 lines; aim for 5–10. | SHOULD (0.38, 0.25) | SHOULD |  |
| c16 | 169 | 2. **Do one thing** — if you can extract a sub-function with a name that isn't just a… | plain (0.87, 0.25), re-asked; first run SHOULD (0.34, 0.28) | plain | backfill 2026-09-25 (prompt-audit F2, keyword on a second copy): the rewrite had made the heading "A function SHOULD do one thing", a second SHOULD on the L1 table's Small-functions rule (c9). Re-asked as a restatement, Jev says plain; the heading reads "Do one thing" again and c9 carries the SHOULD |
| c17 | 171 | 3. **One level of abstraction per function** — you SHOULD NOT mix high-level policy with… | SHOULD (0.31, 0.28) | SHOULD |  |
| c18 | 173 | 4. **No flag arguments** — `render(true)` hides two behaviors; you SHOULD split into… | SHOULD (0.35, 0.29) | SHOULD |  |
| c19 | 175 | 5. **Fewer arguments** — 0 is best, 1 good, 2 fine, 3 needs justification. More th… | SHOULD (0.33, 0.31) | SHOULD |  |
| c20 | 177 | 6. **Command-Query Separation** — change state *or* return a value, not both. | plain (0.83, 0.27), re-asked; first run SHOULD (0.32, 0.29) | plain | backfill 2026-09-25 (prompt-audit F2): "a function SHOULD change state or return a value, not both" was a second SHOULD on the L1 table's No-side-effects rule (c10). Re-asked as a restatement, Jev says plain; the item is a plain imperative again and c10 carries the SHOULD NOT |
| c21 | 181 | Name reveals intent (\`elapsedTimeInDays\`, not \`d\`). | plain (0.75, 0.15) | plain |  |
| c22 | 182 | Pronounceable and searchable; avoid cryptic abbreviations and disinformation. | SHOULD (0.41, 0.20) | plain | style checklist (spec §2: style is plain); uniform with siblings c21/c23/c24 |
| c23 | 183 | Classes are nouns (\`Customer\`, \`Account\`); methods are verbs (\`postPayment\`, \`save… | plain (0.70, 0.16) | plain |  |
| c24 | 185 | No type encodings (\`strName\`, \`iCount\`), no noise words (\`theData\`, \`aInfo\`). | plain (0.50, 0.15) | plain |  |
| c25 | 186 | One word per concept across the codebase — pick \`get\` *or* \`fetch\` *or* \`retrieve… | SHOULD (0.34, 0.19) | plain | style checklist (spec §2: style is plain); uniform with siblings c21/c23/c24 |
| c26 | 191 | A design is simple to the extent that it: | plain (0.94, 0.19) | plain |  |
| c27 | 192 | 1. **Passes all the tests** — it works, verifiably. | MUST (0.53, 0.51) | plain | defines Kent Beck's four rules descriptively, like siblings c26/c28-31; low confidence (0.53) and no gate enforces this restatement (the project's own test suite already governs whether tests pass) |
| c28 | 193 | 2. **Reveals intent** — names and structure say what it does. | plain (0.43, 0.25) | plain |  |
...
| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 30 | Walk a learner — the user, or an agent onboarding onto an unfamiliar system — thro… | plain (0.69, 0.43) | plain |  |
| c2 | 38 | **Locating this skill's helpers (do this first).** The steps below run bundled sc… | MUST (0.79, 0.73) | MUST | the SKILL.md \`$SKILL_DIR\` convention is linted by asset-paths.sh; the subagent hand-off itself is not gate-checked |
| c3 | 100 | 4. **Check understanding — after the walkthrough, never before.** A few targeted … | plain (0.47, 0.33) | plain | Workflow step 4; out of scope per the spec's "workflow steps stay plain imperatives" |
| c4 | 138 | **STALE** → the source moved (new commits, revised doc). Say so before relying on… | plain (0.28, 0.53) | plain | "never trips STALE" describes \`okf.py\`'s diff-detection behaviour, not a directive |
| c5 | 184 | the **reference explainer** — standalone, revisitable, sharable; useful to the le… | plain (0.63, 0.29) | plain | "never saw this session" describes a hypothetical colleague, not a directive |

## tmux-agent-herdr-lite (6 candidates · 1 MUST · 1 SHOULD · 0 MAY · 4 plain · 2 departures)

| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 16 | **Locating this skill's helpers (do this first).** The steps below run bundled s… | MUST (0.76, 0.78) | MUST | the SKILL.md \`$SKILL_DIR\` convention is linted by asset-paths.sh; the subagent hand-off itself is not gate-checked |
...
| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 21 | The division of labor is the point: \`assets/audit_posture.py\` finds candidate de… | plain (0.53, 0.66) | plain |  |
| c2 | 26 | **Locating this skill's helpers (do this first).** The steps below run bundled s… | MUST (0.79, 0.74) | MUST |  |
| c3 | 41 | You MUST state these in the report so it cannot be over-read: | MUST (0.32, 0.53) | MUST |  |
| c4 | 43 | - **Not a CVE scanner.** No advisory database, no network, no version-vulnerabil… | MUST (0.45, 0.54) | plain | describes what the scanner does not do (no advisory DB, no network); not a directive |
| c5 | 45 | - **Not SAST.** No dataflow, taint, or injection analysis. | MUST (0.45, 0.68) | plain | describes what the scanner does not do (no dataflow/taint analysis); not a directive |
| c6 | 46 | - **Not a secrets-content scanner.** It flags credential-*shaped files by name* … | MUST (0.59, 0.69) | plain | describes the scanner's name-only behaviour ("it never reads for secret values"); the pairing tip is advice |
| c7 | 49 | - **Not norms/claims verification** (that is \`base-in-reality\`) and **not an age… | MUST (0.80, 0.80) | plain | names sibling skills for out-of-scope work; a scope description, not a directive |
| c8 | 52 | You MUST audit only repositories the user owns or is explicitly authorized to review; if… | MUST (0.90, 0.87) | MUST |  |
| c9 | 119 | \| \`credential-file\` (.env*/.pem/id_* presence, content never read) \| HIGH \| | MUST (0.61, 0.59) | plain | table row describing the check's behaviour (content never read), not a directive |
...
| c7 | 117 | 6. **Synthesize.** Before filling the report, lint the merged findings array wit… | MUST (0.68, 0.80) | MUST |  |
...
| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 35 | **Locating this skill's helpers (do this first).** The steps below run bundled s… | MUST (0.81, 0.77) | MUST |  |
| c2 | 79 | 1. **Detect the stack.** Run \`python3 "$SKILL_DIR/assets/detect_stack.py" <repo>… | plain (0.61, 0.49) | plain |  |
| c3 | 103 | 3. **Install per the playbook.** For each approved missing rail, follow the matc… | MUST (0.35, 0.64) | MUST |  |
...
| c6 | 161 | - **One ground truth.** Local `verify` and CI MUST run the same commands; have CI call… | MUST (0.41, 0.58) | MUST | (no departure) prompt audit 2026-09 (D-F5): the "when in doubt" hedge became the plain instruction to have CI call the `verify` entrypoint, with its reason; level unchanged |
| c7 | 163 | - **Prove, don't presume.** A rail counts as installed when it was watched faili… | MUST (0.58, 0.55) | MUST |  |
| c8 | 166 | - **Stay off the style battlefield.** Wire checks for whatever formatter/tooling… | SHOULD (0.29, 0.38) | SHOULD |  |
...
| c10 | 172 | This skill follows [skill-contract v1](…). It consumes autonomy grants, which only lift step 2's confirmation, … | not judged (new in 1.2.0, autonomy-grant Task 7) | plain | describes the skill-contract adoption, like crafting-self-prompting-loops c10; the rule it points at is carried by c5 and c9 |

## agent-ready-rails (7 candidates · 4 MUST · 1 SHOULD · 0 MAY · 2 plain · 3 departures)

| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 22 | **Locating this skill's helpers (do this first).** The steps below run bundled s… | MUST (0.86, 0.74) | MUST |  |
| c2 | 43 | **Tier 1 — Build rails (author → merge).** Always audited. These decide whether … | MUST (0.55, 0.70) | plain | "Always audited" describes Tier 1's scope; the scoping directive itself is step 1, a plain workflow step |
...
| c5 | 81 | It walks the target repo read-only and emits sorted JSON evidence per Tier-1 rai… | MUST (0.86, 0.71) | plain | describes what collect_evidence.py does (collects and flags, never scores; settings-error instead of crashes); not a directive |
| c6 | 83 | Then, for each rail in scope, verify and extend the collector's leads against th… | MUST (0.95, 0.79) | MUST |  |
...
| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 25 | **Locating this skill's helpers (do this first).** The steps below run bundled … | MUST (0.77, 0.78) | MUST | |
...
| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 37 | **Locating this skill's helpers (do this first).** The steps below run bundled s… | MUST (0.75, 0.76) | MUST | the SKILL.md `$SKILL_DIR` convention is linted by asset-paths.sh; the subagent hand-off itself is not gate-checked |
| c2 | 52 | - **The bundle is read-only.** The generator MUST NOT mutate the source bundle. When the… | MUST (0.60, 0.59) | MUST | |
| c3 | 55 | - **Tolerance over rejection.** Per the OKF spec's consumer rules, unknown types… | MUST (0.80, 0.77) | MUST | |
| c4 | 59 | - **Spec tracking.** The generator targets OKF v0.1. If the spec has moved (che… | MUST (0.38, 0.64) | SHOULD | fix round 1: confidence 0.38 < 0.6, decided ourselves; `references/okf-spec.md:44-51` itself allows generating without updating — "Before generating ... when network access exists — and always when a bundle declares an okf_version other than 0.1, check the spec URL" — and its own invariant is "Never silently emit ... the WARN: report exists so nothing is dropped without a trace", not "always update the generator first". The check is conditional (network access, or a mismatched okf_version), so a blanket MUST would overclaim; SHOULD fits a strong default with that legitimate condition |
| c5 | 107 | - **Producer-extended bundles** (e.g. `feynman-walkthrough`'s explainers): arbit… | MUST (0.31, 0.61) | plain | describes the generator's rendering behavior for extra frontmatter keys; a descriptive never, not a directive (confidence 0.31 < 0.6) |
...
| c14 | 160 | You MUST wait for the user's explicit yes before running: `write_grant.py …` | not judged (new in 2.0.0, autonomy-grant Task 6) | MUST | a grant is the user's recorded yes (design spec §4 unattended mode); writing one without it is the forgery the threat model names; eval checks the text |
...
| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 34 | **Locating this skill's helpers (do this first).** The steps below run bundled … | MUST (0.66, 0.66) | MUST | the SKILL.md `$SKILL_DIR` convention is linted by asset-paths.sh; the subagent hand-off itself is not gate-checked |
...
| c4 | 61 | - **Boundaries.** This skill explains failures that already happened. Live deb… | plain (0.38, 0.44) | plain | scope statement; plain like every other skill's boundary (plan ruling reversed at final review) |

## knowledge-gardener (2 candidates · 1 MUST · 1 SHOULD · 0 MAY · 0 plain · 1 departure)

| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 33 | **Locating this skill's helpers (do this first).** The steps below run bundled … | MUST (0.81, 0.77) | MUST | the SKILL.md `$SKILL_DIR` convention is linted by asset-paths.sh; the subagent hand-off itself is not gate-checked |
...
| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 31 | **Locating this skill's helpers (do this first).** The steps below run bundled … | MUST (0.81, 0.73) | MUST | the SKILL.md `$SKILL_DIR` convention is linted by asset-paths.sh; the subagent hand-off itself is not gate-checked |

## ai-migration-operating-model (4 candidates · 3 MUST · 0 SHOULD · 0 MAY · 1 plain · 0 departures)

| id | line | sentence | Jev level (conf, harm) | final | departure reason |
|---|---|---|---|---|---|
| c1 | 34 | **Locating this skill's helpers (do this first).** The steps below run bundled … | MUST (0.81, 0.75) | MUST | the SKILL.md `$SKILL_DIR` convention is linted by asset-paths.sh; the subagent hand-off itself is not gate-checked |
| c2 | 135 | - **Not the bulk executor.** This skill ends when the machine is built and the … | plain (0.50, 0.49) | plain | describes the skill's scope boundary, not a directive — matches the security-posture-audit/spec-first-planning Boundaries precedent |
...
| c2 | 45 | If your harness refuses writes to an agent instruction file (Claude Code auto mode treats `~/.claude/CLAUDE.md` as self-modification), you MUST NOT route around it. | MUST (0.83, 0.74) | MUST |  |
