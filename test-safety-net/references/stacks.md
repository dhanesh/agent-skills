...
8. **Platforms.** darwin and linux only. The macOS floor (1.82) is run by CI's one `macos-latest`
   `versions` leg (arm64); it links and passes there. A host's SDK can still refuse an old
   ...
   (NO BUILD), never GREEN. x86_64 macOS is unproven; amd64 Linux is run by CI's `versions`
