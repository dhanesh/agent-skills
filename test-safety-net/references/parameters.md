...
python3 "$SKILL_DIR/assets/rank_risk.py" <repo> [--top-n N] [--since "6 months ago"] \
  [--stack python|node|go] [--no-precise]
```
...
| `name` | The function or class name; for a Go method, `Type.Method`. |
| `lineno` | Line the unit's declaration starts on. |
| `kind` | `"function"` or `"class"` on python and node; `"function"` or `"method"` on go. |
