...
Runtime note (Claude API, models that preserve thinking across turns, e.g. Claude Opus 5.5): if the harness continues one conversation across rounds rather than sending a fresh request each round, carry new state forward by appending it (a new user turn or a mid-conversation system message). Do not edit the system prompt, the `tools` array, or earlier turns: replayed thinking blocks are bound to that exact prefix, and an edited prefix is a 400 on accounts where the binding is enforced. Compact by replacing the whole history with a summary, or with server-side compaction, not by summarizing older turns while replaying newer ones with their thinking blocks.
