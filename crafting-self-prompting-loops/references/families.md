...
- *The gate is a fallible component — assume it degrades.* An unaided human gate loses accuracy exactly when it matters: reviewers grow *more* confident after a cursory look even when wrong, and agree with the agent *more* as it looks more capable (*Confirmation bias*, AAAI 2025). Fight it: give a concrete pass/fail bar (not "looks good?"), and surface the strongest **disconfirming** evidence / arguments for *both* sides — the one intervention shown to help when the model was wrong. As models scale, the human's knowledge edge erodes, so don't lean on the gate as the sole catch — pair it with an external verifier where one exists (§F).
