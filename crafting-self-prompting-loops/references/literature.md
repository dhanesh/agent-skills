...
- **[E/C] Debate helps verifiable reasoning, but cost scales with rounds×agents.** Du et al., *Multiagent Debate*, ICML 2024 (arXiv:2305.14325). → **Design rule:** use debate only for verifiable reasoning/factuality; keep rounds×agents small — Du et al.'s *experiments* typically use ~3 agents over ~2–3 rounds (a setup detail, not a claim in the abstract); some later work disputes its edge over a strong single agent with self-consistency — treat the win as task-dependent. (LSC-9)
