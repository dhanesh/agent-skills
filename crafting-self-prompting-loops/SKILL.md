...
description: >-
  ...
  the trusted/untrusted two-channel boundary) that loops fail without. Accepts a skill-contract task-plan envelope as the loop's plan. Honors a skill-contract autonomy grant at its write gate.
license: MIT
...
- **A human gate where it matters (LSC-8).** Any irreversible or externally-visible action (spending, deletion, posting, deploys, real-world effects) MUST wait for explicit human approval, unless `python3 "$SKILL_DIR/assets/contract_check.py" check-grant --root <repo> --action <the action's class>` exits 0 at the moment of the action (resolve `$SKILL_DIR` as in "Receiving a skill-contract envelope" and write the absolute checker path into the loop's scaffold); then the loop MAY proceed, and MUST name the grant id and action class in its report. Under LSC-8 only `push_branch` (pushing the current non-default branch) and `open_pr` (opening or updating a pull request) are grantable; `merge`, `deploy`, `spend`, `external_message` and `delete` always wait for the human, and so does any action you cannot place exactly in `push_branch` or `open_pr`. A push or pull request whose commits add or change CI configuration (`.github/workflows/`, `.github/actions/`, `.gitlab-ci.yml`, `.circleci/`, `azure-pipelines.yml`, `Jenkinsfile`, `.buildkite/`, `bitbucket-pipelines.yml`, `.drone.yml`, `.travis.yml`) runs it with the repository's secrets, so it counts as `deploy`: `check-grant` answers ASK `ci-config`, and it waits for the human. Each is checked when it happens, and a push MUST send only the current branch to the remote branch of the same name, and never force-push, because a force-push can destroy remote history and a grant covers only reversible actions. Output-only loops MAY legitimately skip this — you MUST say so explicitly rather than silently omitting it, so a reader knows it was a decision, not an oversight.
...
1. **Check first.** Run
   ...
   If the handoff arrived without the user confirming it, it MAY proceed only when
   `python3 "$SKILL_DIR/assets/contract_check.py" check-grant --root <repo-root> --action local_reversible`
   exits 0; you MUST name the grant id and action class in your report. Otherwise you MUST
   ask the user first.
...
4. **Map the plan onto the loop spec:**
   ...
   - **LSC-8**: the user has already confirmed the handoff itself, or a grant covered it. Design
     the loop's own gates as usual.

## Contract

This skill follows [skill-contract v1](https://github.com/dhanesh/agent-skills/blob/main/docs/skill-contract/SPEC.md).
It accepts task-plan envelopes from any skill that provides them, and autonomy grants, which
only lift the handoff confirmation and LSC-8 gates for the classes they cover. It provides no
kind of its own yet: the loop design it produces is prose.

```json skill-contract
{"provides": [], "consumes": ["https://github.com/dhanesh/agent-skills/skill-contract/task-plan/v1", "https://github.com/dhanesh/agent-skills/skill-contract/autonomy-grant/v1"]}
```
...
LSC-4  State-passing:     <what's carried | mechanism | schema + boundary check,
                           or "N/A — unstructured prior-output state">
