...
`validate-skill.sh`, `scan-leaks.sh`, `dry-run-replay.sh`, and `package-skill.sh` are vendored copies — to
update them, re-copy from `repo2skill/scripts/` and note the sync in the commit message. `prompting-playbook.sh`
is repo-local (not vendored); it derives from the talk, not from `repo2skill`.
