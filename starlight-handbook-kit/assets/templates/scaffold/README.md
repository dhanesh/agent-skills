...
make verify  # build + run all 9 integrity gates (what CI runs)
```
...
- `src/content.config.ts` — Starlight docs schema, extended with
  `last_reviewed` (date), `volatility` (`stable` | `volatile`, sets the
  freshness review window), and `decision_guidance` (boolean) frontmatter.
...
- `scripts/check-*.mjs` — nine dependency-free integrity gates run by
  `npm run verify`.
...
2. Fill in the frontmatter (`title`, `description`, `last_reviewed`, and
   optionally `volatility`) and each fixed `##` section.
