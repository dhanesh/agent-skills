...
{/*
  ...
  `last_reviewed` (ISO YYYY-MM-DD) is REQUIRED — the freshness gate fails the
  build if it is missing or malformed. `volatility` sets the review window:
  `volatile` (the default if you omit it) wants a review every 12 months,
  `stable` every 24. Going past the window is a non-blocking WARNING, not a
  failure — keep the date current as you revise the page.
...
Diagrams go in the `<Mermaid>` component, not a fenced code block: Starlight's
Expressive Code renders a fenced mermaid block as syntax-highlighted *code*, not a
diagram, and the mermaid gate fails the build on one.
...
{/* <DecisionTree client:visible root={ ...see DecisionTree.tsx props... } /> */}
