...
| [`harvard-case-method`](harvard-case-method/) | Aid and coach for a real business or product decision: the agent takes the assembly labour (research, base rates, sourcing) while the user keeps the judgement work, split along Stanford's Decision Quality chain and scored as its weakest link. Refuses a brief with two options, uncited figures or no base rate, and a decision record missing a premortem, a falsifier, or dated probability forecasts — which resolve later into a real Brier/calibration profile. Drills historical cases with the ending sealed for fast reps. |
