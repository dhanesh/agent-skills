...
| [`decision-triage`](decision-triage/) | Break decision paralysis with a diagnosis-first protocol instead of a menu of frameworks: classify reversibility and reversal cost, diagnose which of five triggers is live (choice overload, perfectionism, regret avoidance, low confidence, ambiguous ownership), route to a fast or full path, and close with a linted decision record. Structurally lean-resistant — the shipped linter fails any record whose verdict precedes its criteria, the one defect an agreeable model reliably produces. Kernel distilled from 46 candidate methods with the kill decisions recorded; ships a record linter and a Brier calibration scorer. |
