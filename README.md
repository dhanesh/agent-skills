...
| [`okf-capability-catalog`](okf-capability-catalog/) | Externalises cross-team capability contracts and dependency edges into an Open Knowledge Format (OKF v0.1) bundle in git, so an unwritten dependency stops being a silently accepted risk. Scans service repos into Capability/Service documents, interviews the consuming and providing teams for what code cannot know (date, consequence, fallback), computes effective readiness across the hard-requires closure with a `depth` qualifier so "nobody has looked" never renders as green, trips a dependency automatically at its point of no return and propagates the trip downstream, and enforces at the commit boundary that no team certifies its own work. |
