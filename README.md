...
| [`test-safety-net`](test-safety-net/) | Authors unit tests into a codebase that has none, so an agent can change it safely. Ranks units by blast radius and churn, then writes characterization tests that pin current behaviour — each one proved able to FAIL before it is kept, so the suite is a real change-detector, not green noise. Code that dials out (network/DB/filesystem in the constructor) is triaged, never netted with a mocked test; it becomes a ranked seam list for `clean-code`. Fills the loop `verifier-installer` installs. |
