...
| [`desirable-difficulty`](desirable-difficulty/) | Evidence-based learning coach — judges every study activity by one diagnostic test (does it force generation/retrieval under difficulty?), prescribes the methods with the strongest research support (retrieval practice, spacing, interleaving, pretesting, self-explanation), and redirects away from fluency-illusion habits (rereading, highlighting, "learning styles"). Emits a concrete learning plan with an expanding-interval spaced-retrieval schedule; includes a codebase/system-onboarding playbook (trace → reconstruct-from-memory → 5 Whys). |
