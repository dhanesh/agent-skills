...
| [`harvard-case-method`](harvard-case-method/) | Runs a real decision-forcing case study instead of the retrospective success narration an AI defaults to: pins the record to a decision date, withholds the outcome, makes the learner commit a call with its disconfirming evidence and a falsifier, and only then reveals and scores on process rather than result. Six lint rules refuse a case that leaks post-decision dates, uses hindsight language, cites no source for a figure, or names no comparator that failed — the hindsight and survivorship guards made mechanical. |
