...
| [`project-atlas`](project-atlas/) | Index every project directory on a local machine — and every agent session run in them — into one durable SQLite database: path, git remote/branch/HEAD, languages, toolchains, plus chat transcripts stored verbatim with the task state a resume needs. FTS5 full-text search across projects and chats alike (LIKE fallback where the build has no FTS5), replicated to S3, Cloudflare R2, Backblaze B2 or local disk via Litestream 0.5.x with a generated restore runbook. Restores a captured session onto a different machine, recomputing the agent's project directory and rewriting every record's path so it can be resumed there. Vanished projects are tombstoned rather than deleted; credential-shaped strings are redacted from transcripts by default and the agent's account config is never read; generated configs are pinned to Litestream's current config grammar and carry no secrets. |
